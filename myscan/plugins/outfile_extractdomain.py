...
# @Time    : 2020-03-17
# @Author  : caicai
# @File    : outfile_extractdomain.py
from myscan.lib.core.data import logger, paths
...
class plugin():
    ...
    def run(self):
        ...
        request_raw = base64.b64decode(self.dictdata.get("request").get("raw")).decode("utf-8",
                                                                                       errors="ignore")
        response_raw = base64.b64decode(self.dictdata.get("response").get("raw")).decode("utf-8",
                                                                                         errors="ignore")
        alldomains = []
        for text in (request_raw, response_raw):
            alldomains += self.getdomain(text)
        alldomains = list(set(alldomains))
        actions = []
        for domain in alldomains:
            if not self.existfromredis(domain):
                actions.append(
                    [time.strftime("%Y-%m-%d", time.localtime()), domain, self.gettopdomain(domain)]
                )
        self.outtofile(actions)
        logger.debug("extractdoman insert {} lines".format(len(actions)))
    ...
    def getdomain(self, text):
        ...
        domain_reg = "[^-](([a-z0-9\-]{2,64}\.){2,22}%s)[^a-z0-9\-]" % domain_suf
        for domain in re.findall(domain_reg, text):
            if len(domain[0]) < 64:
    ...
    def outtofile(self, lines):
        self.filename = "extractdomain_{}.csv".format(time.strftime("%Y-%m-%d", time.localtime()))
        filepath = os.path.join(paths.MYSCAN_REPORT_PATH, self.filename)
        exist = True if os.path.exists(filepath) else False
        headers = ["time", "domain", "topdomain"]
    ...
    def existfromredis(self, domain):
        if not self.red.sismember(self.redis_key, domain):
            self.red.sadd(self.redis_key, domain)
            return False
        return True
