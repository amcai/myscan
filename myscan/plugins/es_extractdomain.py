...
class plugin():
        ...
        for domain in alldomains:
            try:
                action = {
                    "_index": "burpdns",
                    "_type": "doc",
                    "_id":self.get16md5(domain),
                    "_source": {
                        "domain":domain,
                        "topdomain":self.gettopdomain(domain),
                        "ts":int(time.time())
                    }
                }
                actions.append(action)
            except Exception as ex:
                print("Process domain error:{}".format(ex))
        if actions:
    ...
    def get16md5(self,data):
        hash_md5 = hashlib.md5(data.encode())
        return hash_md5.hexdigest()[8:-8]
